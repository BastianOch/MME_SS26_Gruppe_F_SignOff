...
app.post("/api/auth/register", async (req, res) => {
  ...
  try {
    ...
    // Studenten bekommen direkt ein eigenes Projekt
    if (role === "STUDENT") {
      const projectResult = await pool.query(
        ...
        RETURNING id`,
        [`Bachelorarbeit von ${name}`, "ACTIVE"],
      );

      const projectId = projectResult.rows[0].id;

      // Student als Mitglied seines Projekts eintragen
      await pool.query(
        `INSERT INTO project_members
        (project_id, user_id, role)
        VALUES ($1, $2, $3)`,
        [projectId, result.rows[0].id, role],
      );
    }
...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedTypes.includes(file.mimetype)) {
      ...
      const error = new Error("Nur PDF- und DOCX-Dateien sind erlaubt.");
...
app.post("/api/meetings", authenticateToken, async (req, res) => {
  const { projectId, title, date, notes, feedback, status } = req.body;
...
app.patch("/api/meetings/:id", authenticateToken, async (req, res) => {
  ...
  const { title, date, notes, feedback, status } = req.body;
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE meetings
            ...
            WHERE id = $6
            RETURNING *`,
      [title, date, notes, feedback, status, meetingId],
    );
  ...
  try {
    ...
    if (assigneeId) {
      ...
      if (assigneeCheck.rows.length === 0) {
        return res.status(400).json({
          message:
            "Der ausgewählte Bearbeiter ist kein Mitglied dieses Projekts.",
        });
      }
    }
    const result = await pool.query(
...
app.patch("/api/tasks/:id", authenticateToken, async (req, res) => {
  ...
  const { assigneeId, title, description, deadline, status } = req.body;
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE tasks
            ...
            WHERE id = $6
            RETURNING *`,
      [assigneeId, title, description, deadline, status, taskId],
    );
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO milestones
            ...
            VALUES ($1, $2, $3, $4, $5)
            RETURNING *`,
      [projectId, title, description || null, deadline, status],
    );
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE milestones
            ...
            WHERE id = $5
            RETURNING *`,
      [title, description, deadline, status, milestoneId],
    );
...
app.post("/api/signoffs", authenticateToken, async (req, res) => {
  const { milestoneId, decision, comment } = req.body;
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO sign_offs
            ...
            RETURNING *`,
      [milestoneId, userId, decision, comment || null],
    );
  ...
  try {
    const result = await pool.query(
      `UPDATE sign_offs
            ...
            AND user_id = $4
            RETURNING *`,
      [decision, comment, signOffId, userId],
    );
...
app.get(
  "/api/meetings/:id/document-versions",
  authenticateToken,
  async (req, res) => {
    const meetingId = req.params.id;
    const userId = req.user.userId;
...
app.post("/api/document-versions", authenticateToken, async (req, res) => {
  const { documentId, meetingId, filePath } = req.body;
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO document_versions
            ...
            VALUES ($1, $2, $3, $4, $5)
            RETURNING *`,
      [documentId, meetingId || null, uploaderId, nextVersion, filePath],
    );
...
app.post(
  ...
  upload.single("file"),
  async (req, res) => {
    const { documentId, meetingId } = req.body;
